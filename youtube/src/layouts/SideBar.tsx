import { Home, Repeat, Clapperboard, Library, ChevronUp, ChevronDown, History, PlaySquare, Clock, ListVideo, Flame, ShoppingBag, Music2, Radio, Newspaper, Podcast, Trophy, Lightbulb, Shirt } from "lucide-react"
        ...
        <>
            <aside className={`sticky top-0 overflow-y-auto scrollbar-hidden pb-4 flex flex-col ml-1 ${
                ...
                <LargeSidebarSection visibleItemCount={5}>
                    ...
                    {playlists.map(playlist => (<LargeSidebarItem key={playlist.id}
                ...
                <LargeSidebarSection title="Subscriptions">
                    {subscriptions.map(subscription => (<LargeSidebarItem key={subscription.id}
                        IconOrImgUrl={subscription.imgUrl} title={subscription.channelName} url={`/@${subscription.id}`}
                    />))}
                </LargeSidebarSection>
                ...
                <LargeSidebarSection title="Explore">
                    ...
                    <LargeSidebarItem IconOrImgUrl={Lightbulb} title="Learning" url="/learing" />
...
function LargeSidebarItem({ IconOrImgUrl, title, url, isActive = false }: LargestSidebarItemProps) {
    return (
        <a href={url} className={twMerge(buttonStyles({
            variant: "ghost"
        }), `w-full flex items-center rounded-lg gap-4 p-3 ${isActive ? "font-bold bg-neutral-100 hover: (var[--bg-secondary])" : undefined}`
        )}>
